...
class PtxboaProcessType(PtxboaTypeBase):
    ...
    def create(self, get_data: Callable, main_flow_out: "PtxboaFlow") -> PtxboaProcess:
        """Create instance."""
        # check
        if not main_flow_out.dtype == self.main_flow_type_out:
            raise TypeError(f"{main_flow_out.dtype} != {self.main_flow_type_out}")
...
@dataclass(frozen=True, slots=True)
class PtxboaProcessTree(PtxboaProcessType):
    """Parameter instance variable."""
    ...
    processes: tuple[PtxboaProcess, ...]

    def __post_init__(self):

        # TODO: this might not work if its an actual graph
        first_process = self.processes[0]  # noqa
        last_process = self.processes[-1]  # noqa
    ...
    def __str__(self) -> str:
        return f"{self.code}({', '.join(str(x) for x in self.processes)})"
...
class PtxboaProcessTreeType(PtxboaProcessType):
    ...
    def create(
        ...
        main_flow_in = process.main_flow_in

        # calculated eff

        eff = PtxboaParameter(
            dtype=ptxboa.classes._generated.PtxboaParameterTypes.EFF,
            value=main_flow_out.value / main_flow_in.value,
        )

        return PtxboaProcessTree(dtype=self, eff=eff, main_flow_out=main_flow_out)
